...
export class DiscoveryService {
  ...
  private readonly PREFIX = 'discovery:ip:';
  private readonly REFRESH_TTL = 30; // Seconds before device is removed
   ...
   */
  async announceDevice(ip: string, device: Device): Promise<void> {
   ...
   */
  async getPeers(ip: string): Promise<Device[]> {
    const key = this.PREFIX + ip;
   ...
   */
  async removeDevice(ip: string, deviceId: string): Promise<void> {
    const key = this.PREFIX + ip;
